...
export const details = {
    name: "notion_manager",
    description: "Công cụ quản lý bộ nhớ ngoại vi: Lưu/Đọc nhiệm vụ (Tasks) từ Notion, hoặc ĐỌC KIẾN THỨC CỐT LÕI (Search pages) được nhúng trong thư viện.",
    parameters: {
        type: "object",
        properties: {
            action: {
                type: "string",
                enum: ["add_task", "get_tasks", "search_knowledge"],
                description: "Hành động: 'add_task', 'get_tasks', 'search_knowledge' (tìm tài liệu/wiki trong Workspace)",
            },
            query: {
                type: "string",
                description: "Từ khóa tra cứu kiến thức (Bắt buộc nếu action='search_knowledge')",
            },
            title: {
                type: "string",
...
export async function execute(args: { action: string; title?: string; due_date?: string; query?: string }, deps: any, ctx: any) {
    ...
    const { action, title, due_date, query } = args;
    ...
    try {
        if (action === "add_task") {
            ...
            if (!query) return JSON.stringify({ status: "error", message: "query is required for search_knowledge" });
            
            await ctx?.sendProgress?.(`Đang lặn vào thư viện Notion tìm kiếm: "${query}"...`);
            
            const searchResp = await notion.search({
                query: query,
                filter: { value: "page", property: "object" },
                sort: { direction: "descending", timestamp: "last_edited_time" },
                page_size: 3 // Lấy tối đa 3 tài liệu sát nhất để tránh tràn RAM
            });
            
            if (searchResp.results.length === 0) {
               return JSON.stringify({ status: "success", message: `Không tìm thấy tài liệu Notion nào trong kho học liệu liên quan đến "${query}".`});
            }
            
            let docs = [];
            for (const page of searchResp.results as any[]) {
               try {
                  const blocks = await notion.blocks.children.list({ block_id: page.id, page_size: 60 });
                  
                  let textContent = "";
                  for (const block of blocks.results as any[]) {
                     if (block[block.type]?.rich_text) {
                        const texts = block[block.type].rich_text.map((rt: any) => rt.plain_text).join("");
                        textContent += texts + "\n";
                     }
                  }
                  ...
                  if (page.properties) {
                    const titleKey = Object.keys(page.properties).find(k => page.properties[k].type === "title");
                    if (titleKey && page.properties[titleKey].title[0]) {
                       pageTitle = page.properties[titleKey].title[0].plain_text;
                    }
                  }
                  
                  docs.push(`[Tài liệu: ${pageTitle}] (ID: ${page.id})\n>> Trích xuất lõi:\n${textContent.substring(0, 2000)}`);
               } catch(ex) {
                  continue; 
               }
            }
...
export default {
    ...
    input_schema: details.parameters,
    execute: execute
};
